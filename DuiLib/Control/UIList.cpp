...
BOOL CListUI::SetHeaderItemData(int nColumn, CControlUI* pControl)
{
	bool bok = false;
	CListHeaderUI *pHeader = CListUI::GetHeader();
	if ( pHeader!=NULL) {
...
CListContainerElementUI* CListUI::GetListItemElement(int iIndex)
{
...
int CListUI::InsertItem(int nItem, int nItemHeight/* = 20*/, CListContainerElementUI *pListItem/* = NULL*/)
{
	if ( pListItem==NULL ) {
		pListItem = new CListContainerElementUI;
		CListHeaderUI* pHeader = CListUI::GetHeader();
		pListItem->m_pHeader = pHeader;
		if ( pHeader!=NULL ) {
...
bool CListUI::SetSubItem(int nItem, int nSubItem, CControlUI* pControl)
{
	...
	if ( pListItem!=NULL ) {
		pSubHor = static_cast<CHorizontalLayoutUI*>(pListItem->GetItemAt(nSubItem));
		if ( pSubHor!=NULL ) {
