...
LRESULT CUIDesignerView::OnReloadDocumentFile(WPARAM wParam, LPARAM lParam)
{
	...
	if ( pWnd ) {
		//注意要放到WM_CLOSE前，窗口小消失后拿不到Document
...
void CUIDesignerView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView)
{
	if ( bActivate ) {
		CString strFilePath = GetDocument()->GetPathName();
		//if ( strFilePath.Find(_T("pageWrapper.xml"))!=-1 ) {
		//	int a = 0;
		//	a++;
		//}
		...
		if(CFile::GetStatus(strFilePath, status)) {
			CTime timeLastMod = status.m_mtime;//得到修改时间
...
void CUIDesignerView::OnActivateFrame(UINT nState, CFrameWnd* pFrameWnd)
{
	__super::OnActivateFrame(nState, pFrameWnd);
}
