...
public:
	...
	{
		TRACE(_T("%s:%s\n"),pstrName,pstrValue);
		m_pAttributeList[CString(pstrName)]=CString(pstrValue);
	}

	std::map<CString,CString> m_pAttributeList;
protected:
	CString mClassName;
};
									...
									, BOOL bSaveChildren/* = TRUE*/)
{
	...
	{
		std::map<CString,CString>::iterator iter=pUserDefinedControl->m_pAttributeList.begin();
